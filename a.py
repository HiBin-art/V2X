...
if __name__ == '__main__':
    ...
    with g.as_default():
        ...
        # TensorFlow 1.x does not have `config`, so use ConfigProto to set memory growth
        config = tf.ConfigProto()
        config.gpu_options.allow_growth = True  # Enable memory growth for GPU
        ...
        # Train only if the flag is set to true
        if IS_TRAIN:
            for i_episode in range(n_episode):
                print("-------------------------")
                print('Episode:', i_episode)

                # Decreasing epsilon during annealing period
                ...
                if i_episode % 100 == 0:
                    ...
                    env.renew_neighbor()  # Update vehicle neighbors
                ...
                for i_step in range(n_step_per_episode):
                    ...
                    # Step through all vehicles and neighbors
                    for i in range(n_veh):
                        for j in range(n_neighbor):
                            # Get state for this vehicle and neighbor
                    ...
                    # All agents take actions simultaneously and obtain shared reward
                    action_temp = action_all_training.copy()
                    train_reward = env.act_for_training(action_temp)
                    record_reward[time_step] = train_reward

                    # Update environment (channel fast fading, interference calculation)
                    ...
                    for i in range(n_veh):
                        for j in range(n_neighbor):
                            ...
                            # Training mini-batch and loss calculation
            ...
            # Saving models after training is done
