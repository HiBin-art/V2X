...
class Agent(object):
    def __init__(self, memory_entry_size):
        ...
        self.memory = ReplayMemory(self.memory_entry_size)#进入------------------------------
...
if __name__ == '__main__':
    ...
    env = Environment_marl.Environ(down_lanes, up_lanes, left_lanes, right_lanes, width, height, n_veh, n_neighbor)#进入----------------
    env.new_random_game()  # initialize parameters in env进入-------------------------
    #强化学习参数：
    n_episode = 1#3000
    ...
    n_hidden_2 = 250
    n_hidden_3 = 120
    n_input = len(get_state(env=env))#进入------------------------------
    n_output = n_RB * len(env.V2V_power_dB_List)# the power levels
    ...
    with g.as_default():
        ...
        for ind_agent in range(n_veh * n_neighbor):  # initialize agents
            ...
            agent = Agent(memory_entry_size=len(get_state(env)))#进入---------------------
            agents.append(agent)
        ...
        record_loss = []
        if IS_TRAIN:
            for i_episode in range(n_episode):#自己上面指定的，141行
                print("-------------------------")
                print('Episode:', i_episode)
                if i_episode < epsi_anneal_length:
                    epsi = 1 - i_episode * (1 - epsi_final) / (
                                epsi_anneal_length - 1)  # epsilon decreases over each episode
                else:
                    epsi = epsi_final
                if i_episode % 100 == 0:
                    env.renew_positions()  # update vehicle position 进入-------------------------
                    env.renew_neighbor()#进入----------------------------
                    env.renew_channel()  # update channel slow fading进入----------------------
                ...
                for i_step in range(n_step_per_episode):#142行
                    time_step = i_episode * n_step_per_episode + i_step
                    state_old_all = []
                    action_all = []
                    action_all_training = np.zeros([n_veh, n_neighbor, 2], dtype='int32')
                    for i in range(n_veh):#134行
                        for j in range(n_neighbor):#135行
                            state = get_state(env, [i, j], i_episode / (n_episode - 1), epsi)#重点
                            state_old_all.append(state)
                            action = predict(sesses[i * n_neighbor + j], state, epsi)#重点
                    ...
                    train_reward = env.act_for_training(action_temp)#进入-----------------------------------------------
                    record_reward[time_step] = train_reward#进入----------------------

                    env.renew_channels_fastfading()#进入----------------------
                    env.Compute_Interference(action_temp)#进入----------------------
                    ...
                    for i in range(n_veh):
                        for j in range(n_neighbor):
                            ...
                            agents[i * n_neighbor + j].memory.add(state_old, state_new, train_reward,
                                                                  action)  # add entry to this agent's memory

                            # training this agent
                            if time_step % mini_batch_step == mini_batch_step - 1:
                                loss_val_batch = q_learning_mini_batch(agents[i * n_neighbor + j],
                                                                       sesses[i * n_neighbor + j])
                            ...
                            if time_step % target_update_step == target_update_step - 1:
                                update_target_q_network(sesses[i * n_neighbor + j])#进入-----------------------
                                if i == 0 and j == 0:
        ...
        if IS_TEST:
            ...
            for idx_episode in range(n_episode_test):
                ...
                print(round(np.average(V2I_rate_per_episode), 2), 'rand',
                      round(np.average(V2I_rate_per_episode_rand), 2))
