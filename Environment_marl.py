...
class Vehicle:
    # Vehicle simulator: include all the information for a vehicle车辆模拟器：包括车辆的所有信息
...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.V2Vchannels = V2Vchannels()#进入--------------------------------
    ...
    def new_random_game(self, n_Veh=0):
        ...
        if n_Veh > 0:
            self.n_Veh = n_Veh
        self.add_new_vehicles_by_number(int(self.n_Veh / 4))#进入-----------------------
        self.renew_neighbor()#进入-------------------------------------
        self.renew_channel()#进入------------------------比较重要
        self.renew_channels_fastfading()#进入------------------------
