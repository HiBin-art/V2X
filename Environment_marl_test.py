...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        #初始化车辆通信和环境变量,车与车（Vehicle-to-Vehicle, V2V）以及车与基站（Vehicle-to-Infrastructure, V2I）之间的通信通道
        self.V2Vchannels = V2Vchannels()
        self.V2Ichannels = V2Ichannels()
        #初始化一个空列表，用于存储所有的车辆对象
        self.vehicles = []

        #初始化相关的干扰和信号变量
        self.demand = [] #存储每个车辆的通信需求
        # V2V 和 V2I 通信的阴影衰减。阴影衰减是指由于环境中的物体（如建筑物）遮挡信号而导致的信号衰减。
        self.V2V_Shadowing = []
        self.V2I_Shadowing = []
        self.delta_distance = []#存储车辆之间的距离差
        # 两个变量用于存储 V2V 和 V2I 通道的绝对信号强度。
        self.V2V_channels_abs = []
        self.V2I_channels_abs = []

        self.V2I_power_dB = 23  #基站到车辆的 V2I 通信的发射功率，单位是 dBm
        # 包含不同的 V2V 通信功率水平。这个列表表示了不同情况下车辆之间通信的功率级别。
        self.V2V_power_dB_List = [23, 15, 5, -100]
        self.sig2_dB = -114#接收信号的噪声功率，单位是 dBm
        #分别表示基站天线增益和基站的噪声系数
        self.bsAntGain = 8
        ...
        #通过 dBm 转换为线性值的噪声功率（即信号噪声功率）。10 ** (self.sig2_dB / 10) 用于将 dBm 转换为瓦特
        self.sig2 = 10 ** (self.sig2_dB / 10)

        """
        self.n_RB：资源块的数量，这里设置为与车辆数量相等（n_veh）。
        self.n_Veh：车辆数量，用来定义仿真中车辆的总数。
        self.n_neighbor：每辆车的邻居数量，通常是在同一车道上的车辆数目。
        self.time_fast 和 self.time_slow：定义了两种不同时间尺度。time_fast 是更细粒度的更新时间步长（0.001秒），而 time_slow 是较慢的更新时间步长（0.1秒，用于更新车辆位置和慢衰落）。
        self.bandwidth：每个资源块的带宽，单位为赫兹，这里设置为 1 MHz（1e6）。
        self.demand_size：V2V 负载大小。假设每个 V2V 数据包的大小为 1060 字节（每 100 毫秒发送一次），计算其大小时考虑了包头等信息。
        """
        self.n_RB = n_veh
        self.n_Veh = n_veh
        ...
        """
        这三个矩阵用于存储 V2V 通信的干扰信息。
        每个矩阵的大小是 (self.n_Veh, self.n_neighbor, self.n_RB)，
        表示每辆车、每个邻居和每个资源块的干扰值。初始化时，干扰值被设定为 sig2，即噪声功率。
        """
    ...
    def act_for_testing_rand(self, actions):
        """
                                action_temp = actions.copy()：
                                这行代码创建了 actions 的一个副本，并赋值给 action_temp。这样做是为了避免直接修改传入的 actions 参数，以确保原始数据不被修改，保持其不可变性。

                                V2I_Rate, V2V_Rate = self.Compute_Performance_Reward_Test_rand(action_temp)：
                                通过调用 Compute_Performance_Reward_Test_rand 方法来计算 V2I_Rate 和 V2V_Rate。
                                这个方法可能会基于传入的 action_temp 来计算车辆与基站的通信速率（V2I）和车辆之间的通信速率（V2V）。计算结果会被分别赋值给 V2I_Rate 和 V2V_Rate。

                                V2V_success = 1 - np.sum(self.active_links_rand) / (self.n_Veh * self.n_neighbor)：计算 V2V 成功率。
                                self.active_links_rand 是一个数组，表示当前活跃的 V2V 链路。通常，数组中的值可能是 0（表示链路不可用）或 1（表示链路活跃）。
                                np.sum(self.active_links_rand) 会计算活跃链路的总数。
                                (self.n_Veh * self.n_neighbor) 是总共可能的 V2V 链路数量（假设每辆车有一定数量的邻居）。
                                V2V 成功率被定义为 1 - 活跃链路比例，意味着链路越活跃，成功率越高。

                                return V2I_Rate, V2V_success, V2V_Rate：
                                返回计算得到的 V2I 速率、V2V 成功率和 V2V 速率。这些结果将用于测试模型的性能或评估在给定动作下的系统表现。
        """
        action_temp = actions.copy()
        V2I_Rate, V2V_Rate = self.Compute_Performance_Reward_Test_rand(action_temp)
